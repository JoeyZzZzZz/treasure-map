...
# ── a wrapper-recovered ref points at the base ref that holds the provenance ─────────


def test_wrapper_ref_points_to_base(tmp_path: Path) -> None:
    """A ...@cmd_via_wrapper ref names the FORWARDING function, one hop before the real sink; the
    sink_arg record lives on the direct ...@cmd ref. Querying the wrapper ref must say WHERE the
    provenance is, not just "no such ref" (a wrong door read as an absent fact).

    MUTATION (must go RED): delete the ``base_ref`` branch in ``triage.get_sink_provenance``'s
    ``row is None`` arm, so it falls straight through to ``no_such_evidence_ref``. The agent then
    has no way from the ref it holds to the ref that answers."""
    atlas = _seed(tmp_path, ref="run_x#facefeed:000174e4@cmd")
    conn = open_atlas(atlas)
    try:
        res = get_sink_provenance(conn, "run_x#facefeed:000174e4@cmd_via_wrapper", 0)
    finally:
        conn.close()
    assert res["found"] is False
    assert res["note"] == "queried_wrapper_ref"
    assert res["see_base_ref"] == "run_x#facefeed:000174e4@cmd"
    # ★ pointing only: none of the base ref's provenance is merged in or carried across
    assert "record" not in res and "records" not in res


def test_wrapper_ref_points_to_base_on_the_fmt_axis(tmp_path: Path) -> None:
    # The reverse map is derived from _WRAPPER_AXIS, so the fmt axis maps to its own base suffix
    # (fmt_via_wrapper -> fmt_string), not to the cmd one.
    atlas = _seed(tmp_path, ref="run_x#facefeed:000174e4@fmt_string")
    conn = open_atlas(atlas)
    try:
        res = get_sink_provenance(conn, "run_x#facefeed:000174e4@fmt_via_wrapper")
    finally:
        conn.close()
    assert res["see_base_ref"] == "run_x#facefeed:000174e4@fmt_string"


def test_no_baseref_stays_no_such(tmp_path: Path) -> None:
    """When the base ref does not exist either, never point at it: a ref that resolves to nothing
    is a fabricated anchor, which is worse than an honest miss."""
    atlas = _seed(tmp_path, ref="run_x#facefeed:00099999@cmd")  # a DIFFERENT function
    conn = open_atlas(atlas)
    try:
        res = get_sink_provenance(conn, "run_x#facefeed:000174e4@cmd_via_wrapper", 0)
    finally:
        conn.close()
    assert res["found"] is False
    assert res["note"] == "no_such_evidence_ref"
    assert "see_base_ref" not in res


def test_non_wrapper_unknown_ref_unchanged(tmp_path: Path) -> None:
    """A ref without a wrapper suffix keeps the pre-existing behaviour byte for byte."""
    atlas = _seed(tmp_path, ref="run_x#facefeed:000174e4@cmd")
    conn = open_atlas(atlas)
    try:
        res = get_sink_provenance(conn, "run_x#facefeed:000174e4@copy", 0)
    finally:
        conn.close()
    assert res == {
        "evidence_ref": "run_x#facefeed:000174e4@copy",
        "found": False,
        "note": "no_such_evidence_ref",
    }
